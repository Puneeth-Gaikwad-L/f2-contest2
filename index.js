...
function addStudent(student) {
    ...
    tr.innerHTML = `<td class = "center">${student.id}</td>
    <td><img src = ${student.img_src}> ${student.name}</td>
    <td class = "center">${student.gender}</td>
    <td class = "center">${student.class}</td>
    <td class = "center">${student.marks}</td>
    <td class = "center">${student.passing}</td>
    <td class = "center">${student.email}</td>`;
    tbody.appendChild(tr);
}

function addElementsToTable(arr) {
    for (let i = 0; i < arr.length; i++) {
        ...
        let student = {
            ...
            passing: pass,
            email: arr[i].email,
            img_src: arr[i].img_src
        }
        addStudent(student);
    }
}

addElementsToTable(arr);

function removeExistingData() {
   
    let table = document.getElementById("t1");

    let tbodyToRemove = table.querySelector("tbody")

    if(tbodyToRemove){
        table.removeChild(tbodyToRemove);
    }
}
...
function sortAZ() {
    ...
    let newarr = arr.sort(function (a, b) {
        let x = a.first_name; let y = b.first_name;
...
function sortByPassing(){
    removeExistingData();
    let newarr = arr.sort(function (a, b) {
        let x = a.passing; let y = b.passing;
        return ((x < y) ? 1 : ((x > y) ? -1 : 0));
    });
